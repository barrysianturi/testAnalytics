...
data = np.array(df[df.columns[7:]].transpose())
...
sns.kdeplot(df["DIAGNOSTIC TEST"],color="purple", fill=True,ax=axes[0])
axes[0].set_xlim(0.0, 1.0)
axes[0].axvline(df["DIAGNOSTIC TEST"].mean(),linestyle="--",color='purple')
axes[0].axvline(df["DIAGNOSTIC TEST"].median(),linestyle="-.",color='red')
...
corrMatrix = pd.DataFrame(data.transpose()).corr()
